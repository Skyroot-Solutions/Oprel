...
def get_model_context_length(model_path: Path) -> int:
    ...
    Returns:
        Context length in tokens, or default 8192 if not found
    """
    ...
    logger.warning(f"Could not determine context length for {model_path}, using default 8192")
    return 8192
...
def compare_with_ollama_memory(model_path: str) -> Dict[str, Any]:
    ...
    oprel_initial_ctx = min(ctx, 8192)  # Start with 8K context
