...
const MessageRenderer = (() => {
    ...
    /**
     * Extracts text between <think> and </think> tags.
     * Handles streaming cases where the closing tag is missing.
     */
    function _extractThinking(text, isFinal = false) {
        if (!text) return { thinking: '', cleaned: '' };
        ...
        if (startIdx !== -1) {
            let endIdx = text.indexOf('</think>', startIdx + 7);
            
            // Fallback: Check for common malformed closing tags if </think> is missing
            if (endIdx === -1) {
                const malformedTags = ['think>', '</think', '</ think>', '<\think>'];
                for (const t of malformedTags) {
                    // Start searching AFTER the opening <think> tag (7 chars)
                    const mIdx = text.indexOf(t, startIdx + 7);
                    if (mIdx !== -1) {
                        endIdx = mIdx;
                        break;
                    }
            ...
            if (endIdx !== -1) {
                // Completed thinking block (or malformed one we matched)
                // Determine the length of the tag we matched to skip it correctly
                let tagLength = 8; // default for </think>
                const possibleTags = ['</think>', 'think>', '</think', '</ think>', '<\think>'];
                for (const t of possibleTags) {
                    if (text.substring(endIdx, endIdx + t.length) === t) {
                        tagLength = t.length;
                        break;
                    }
                }

                thinking = text.substring(startIdx + 7, endIdx).trim();
                cleaned = text.substring(0, startIdx).trim() + '\n\n' + text.substring(endIdx + tagLength).trim();
            } else {
                // Ongoing thinking (streaming) or missing closing tag
                let rawThinking = text.substring(startIdx + 7);
                
                // HEURISTIC: If </think> is missing but we see a transition to a final answer
                const transitions = [
                    '\n\nFinal Answer:', '\n\nAnswer:', '\n\nConclusion:', 
                    '\n\nTherefore', '\n\nSummary:', '\n\nSo,', '\n\nIn conclusion', 
                    '\n\nOkay,', '\n\nHey', '\n\nHello', '\n\nHi '
                ...
                for (const t of transitions) {
                    const idx = rawThinking.toLowerCase().indexOf(t.toLowerCase());
                    if (idx !== -1) {
                        splitIdx = idx;
                        break;
                    }
                ...
                // Look for common partial closing tags like "think>" (already handled above but as a backup here)
                if (splitIdx === -1) {
                    const partials = ['think>', '</think', '### Response:'];
                    for (const p of partials) {
                        const pIdx = rawThinking.indexOf(p);
                        if (pIdx !== -1) {
                            splitIdx = pIdx;
                            break;
                        }
                ...
                // If stream is finished and still no split, try to find the LAST double newline 
                // if it's followed by significant text (likely the response)
                if (isFinal && splitIdx === -1) {
                    const lastDnl = rawThinking.lastIndexOf('\n\n');
                    if (lastDnl !== -1 && (rawThinking.length - lastDnl) > 20) {
                        splitIdx = lastDnl;
                    }
                }

                if (splitIdx !== -1) {
                    thinking = rawThinking.substring(0, splitIdx).trim();
                    cleaned = text.substring(0, startIdx).trim() + '\n\n' + rawThinking.substring(splitIdx).trim();
                } else {
        ...
        return { thinking: thinking.trim(), cleaned: cleaned.trim() };
    }

    // ─── NEW: Sanitize incomplete markdown during streaming ──────────────────────
    // Called only during streaming (isFinal=false) to close dangling constructs
    // so marked.js doesn't produce raw text for incomplete tokens.
    function _sanitizeForStreaming(text) {
        if (!text) return text;

        // 1. Close unclosed fenced code blocks.
        //    Count opening fences (``` at line start). If odd → add closing fence.
        const fenceMatches = text.match(/^```/gm) || [];
        if (fenceMatches.length % 2 !== 0) {
            text += '\n```';
        }

        // 2. Close an open <think> block so _extractThinking() can parse it
        //    and the thinking UI renders instead of leaking raw tags.
        const thinkStart = text.indexOf('<think>');
        if (thinkStart !== -1 && text.indexOf('</think>', thinkStart + 7) === -1) {
            text += '\n</think>';
        }

        // 3. Close unclosed bold (**) — an odd number of ** markers trips marked.
        //    Simple heuristic: count standalone ** (not inside code spans).
        //    Strip code spans first to avoid false positives.
        const noCode = text.replace(/`[^`]*`/g, '``');
        const boldMarkers = (noCode.match(/\*\*/g) || []).length;
        if (boldMarkers % 2 !== 0) {
            text += '**';
        }

        // 4. Close unclosed italic (*) — same idea, but only single * not part of **
        //    Replace ** with placeholders so we count single * only.
        const noDouble = noCode.replace(/\*\*/g, '');
        const italicMarkers = (noDouble.match(/\*/g) || []).length;
    ...
    function render(text, isFinal = false) {
        if (!text) return '';
        init();
        try {
            // During streaming, patch incomplete markdown constructs first
            const source = isFinal ? text : _sanitizeForStreaming(text);

            const { thinking, cleaned } = _extractThinking(source, isFinal);
            
            // Render the main content
            var normalized = _normalize(cleaned);
            var result = marked.parse(normalized);
            let html = (typeof result === 'string') ? result : String(result || '');
            
            // If there's thinking content, prepend the thinking block
            if (thinking) {
                const thinkingNormalized = _normalize(thinking);
                const thinkingHtml = marked.parse(thinkingNormalized);
                
                // During streaming, show a pulsing "Thinking…" label
                const isStreaming = !isFinal;
                const thinkingBlock = `
                    <div class="oprel-thinking-block expanded">
                        <button class="oprel-thinking-toggle" onclick="this.parentElement.classList.toggle('expanded')">
                            <iconify-icon icon="solar:alt-arrow-down-linear"></iconify-icon>
                            <span>${isStreaming ? '<span class="oprel-thinking-pulse">Thinking…</span>' : 'Thinking'}</span>
                        </button>
                        <div class="oprel-thinking-content">
                            ${thinkingHtml}
                        </div>
                    </div>
                `;
                html = thinkingBlock + html;
            }
