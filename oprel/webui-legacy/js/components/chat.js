...
class Chat {
    ...
    static newConversation() {
        ...
        this.clearImage();
        // Reset mode to fast for new chat
        this.setMode('fast', document.getElementById('mode-btn-fast'));
    }
    ...
    static handleImageSelect(e) {
        ...
        if (typeof Models !== 'undefined') {
            const currentModel = Models.state.current;
            if (currentModel && !Models.isVisionModel(currentModel)) {
                App.showToast(`The current model "${currentModel.name || currentModel.model_id}" does not support images. Please switch to a Vision model.`, 'warning', 'Incompatible Model');
    ...
    static setMode(mode, btn) {
        ...
        // Update UI
        document.querySelectorAll('.oprel-mode-btn').forEach(b => b.classList.remove('active'));
        btn.classList.add('active');
        
        // Show toast for confirmation
        if (typeof App !== 'undefined') {
            const label = mode.charAt(0).toUpperCase() + mode.slice(1);
            App.showToast(`Switched to ${label} mode.`, 'info');
        }
    }
    ...
    static appendMessage(role, content, metrics = null) {
        ...
        if (isLoading) {
            ...
            renderedContent = MessageRenderer.render(content, true);
        }
    ...
    static async send() {
        ...
        // Prepare settings
        const config = (typeof Settings !== 'undefined') ? Settings.state.config : {
            temperature: 0.7,
            top_p: 0.9,
            top_k: 40,
            repeat_penalty: 1.1,
        ...
        const isFastMode = Chat.state.mode === 'fast';
        const isThinkingMode = Chat.state.mode === 'thinking';
        ...
        try {
            ...
            await API.chatCompletionStream({
                ...
                // Fast: cap tokens for speed. Thinking: allow full budget.
                max_tokens: image ? 8192
                        : isThinkingMode ? Math.max(config.max_tokens, 8192)
                        : Math.min(config.max_tokens, 2048),

                // Fast: lower temp = more deterministic = fewer hedging tokens.
                // Thinking: slightly higher for richer reasoning.
                temperature: isFastMode
                    ? Math.min(config.temperature, 0.4)
                    : isThinkingMode
                        ? Math.max(config.temperature, 0.6)
                        : config.temperature,
                ...
                // Signal to backend: true = inject CoT prompt / false = suppress <think>
            ...
            // Final render to resolve any missing tags or heuristics
            contentEl.innerHTML = MessageRenderer.render(buffer, true);
            this._scrollToBottom();

            // 4. Calculate metrics and replace placeholder with final message
