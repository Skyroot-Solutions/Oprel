const App = {
    ...
    async saveUserProfile() {
        ...
        if (!name || !role) {
            this.showToast('Please enter both name and role', 'warning');
            return;
        }
    ...
    showToast(message, type = 'info', title = null) {
        ...
        toast.innerHTML = `
            <iconify-icon icon="${icons[type] || icons.info}" width="24" class="${type === 'info' ? 'text-blue-500' : (type === 'success' ? 'text-green-500' : 'text-red-500')}"></iconify-icon>
